...
$$
f = min_{f \in \mathcal{F}} \frac{1}{n}  \sum_{i=1}^{n} L(y_i, f(x_i))
$$
...
$$
w^T = (\overline{xx^T}) ^ {-1} \overline{yx}
$$
...
Logistic regression is a bit more robust than linear regression. It works good on data that seems linear yet not linear enough for linear regression.

Logistic Regression use function space,
...
$$
L = \frac{1}{n} \sum_{i=1}^{n} (y_i - (w^Tx_i + b))^2 + \lambda ||w||_1
$$
...
$$
L = \frac{1}{n} \sum_{i=1}^{n} (y_i - (w^T x_i + b))^2 + \lambda ||w||_2^2
$$
