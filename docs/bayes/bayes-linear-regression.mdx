...
- $\mathbf{x}_i = (x_{i1}, x_{i2}, \dots, x_{ip})^T$ is the feature vector for the $i$-th observation,
- $\beta = (\beta_0, \beta_1, \dots, \beta_p)^T$ is the vector of unknown regression coefficients (parameters),
